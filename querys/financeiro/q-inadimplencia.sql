...
-- --,100-((SUM(CASE WHEN ICR.situacao = 2 THEN valor ELSE 0.00 END )*100)/CAST(SUM(VALOR) AS NUMERIC(15,2))) AS PERCENTUAL 
...
WITH INADIMPLENCIA AS (
    ...
    CAST(SUM(VALOR) AS NUMERIC(15,2)) AS VALOR_TOTAL,
    SUM(CASE WHEN ICR.situacao = 2 THEN valor ELSE 0.00 END ) AS VALOR_RECEBIDO,
    (SELECT MON$ATTACHMENTS.mon$attachment_name FROM MON$ATTACHMENTS)
    FROM ICR 
    WHERE 1=1
    AND ICR.DTVENCTO BETWEEN DATEADD(-2 YEAR TO CURRENT_DATE) 
    AND CURRENT_DATE 
